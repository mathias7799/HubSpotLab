...
**Note:** This feature requires approval from HubSpot. Submit [this form](https://app.hubspot.com/l/developer-overview/appObjectsEventsRequest) to request the `CloseReady` prefix and `CLOSEREADY_RULE` object name.

This directory is intentionally outside `src/` while approval is pending so normal development uploads can succeed. After HubSpot approves both identifiers, copy `app-object-hsmeta.json` into `src/app/app-objects/` and upload the project. HubSpot will then install and manage the app's single rule object with the app.
