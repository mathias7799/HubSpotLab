...
export async function onHubSpotWebhookEvent(
  ...
  console.info("HandoffReady deal lifecycle event received", {
...
function readEvents(rawBody: string): HubSpotWebhookEvent[] {
  ...
  if (!Array.isArray(value) || value.length === 0 || value.length > 100) {
    throw new HttpError(
      400,
      "Webhook body must contain between 1 and 100 events.",
    );
  }
  return value.map((event) => {
    if (
      ...
      throw new HttpError(
        400,
        "Webhook event is missing required HubSpot fields.",
      );
    }
