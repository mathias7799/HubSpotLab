...
describe("HubSpotObjectConfigurationStore", () => {
    ...
    const mockFetch = vi.fn(
      fetcher(async (input, init) => {
        ...
        if (url.endsWith("/search")) {
          return Response.json({ results: record ? [record] : [] });
        }
        if (method === "POST") {
  ...
  it("coalesces concurrent schema provisioning per portal", async () => {
    ...
    const mockFetch = fetcher(async (input, init) => {
      const url = String(input);
      if (url.endsWith("/crm-object-schemas/v3/schemas") && !init?.method) {
        schemaReads += 1;
  ...
  it("isolates ciphertext by portal and key", async () => {
    ...
    const mockFetch = fetcher(async (input, init) => {
      const url = String(input);
      const method = init?.method ?? "GET";
      if (url.endsWith("/crm-object-schemas/v3/schemas") && method === "GET") {
        return Response.json({
          results: [
            {
              name: "example_app_configuration",
      ...
      if (url.endsWith("/search")) {
        const key = (
          body.filterGroups as Array<{ filters: Array<{ value: string }> }>
        )[0]!.filters[0]!.value;
        const encrypted = records.get(key);
        return Response.json({
          results: encrypted
            ? [{ id: "1", properties: { encrypted_value: encrypted } }]
            : [],
        });
      }
